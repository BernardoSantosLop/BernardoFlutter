...
class _MyHomePageState extends State<MyHomePage> {
  ...
  final url = Uri.parse(
      "https://resnet-service-bernardosantoslop.cloud.okteto.net/v1/models/resnet:predict");
  ...
  Future<void> uploadImage(base64) async {
    try {
      ...
      if (res.statusCode == 200) {
        ...
        String clases_prediction =
            json_prediction['predictions'][0]['classes'].toString();
        log("Clase: $clases_prediction");

        final value =
            await rootBundle.loadString('Data/imagenet_class_index.json');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Resnet APP Bernardo"),
      ),
      body: Center(
          child: Column(
        children: [
